...
class ProductController extends Controller
{
  ...
  public function postProductAdd(Request $request) {
    ...
    $messages = [
      'name.required' => 'Es necesario el nombre del nombre',
      'img.required' => 'Es necesario una imagen representativa',
      'img.image' => 'El archivo no es una imagen',
      'price.required' => 'Indique el precio del producto',
      'content.required' => 'Es necesaria una descripción del producto'
    ];
    ...
    if($validator->fails()):
      ...
      if($product->save()):
        ...
        return redirect('/admin/products/1')->with('message', '¡Producto creado exitosamente!')->with('typealert', 'success');
      endif;
    endif;
  }
  ...
  if($validator->fails()):
    ...
    if($product->save()):
      if($request->hasFile('img')):
        ...
        $img->save($upload_path.'/'.$path.'/t_'.$filename);
        // unlik($upload_path.'/'.$ipp.'/'.$ip);
        // unlik($upload_path.'/'.$ipp.'/t_'.$ip);
      endif;
      return back()->with('message', '¡Producto modificado exitosamente!')->with('typealert', 'success');
    endif;
  endif;
}
...
public function postProductGalleryAdd($id, Request $request) {
  ...
  $messages = [
    'file_image.required' => 'Es necesario una imagen representativa'
  ];
    ...
    if($request->hasFile('file_image')):
      ...
      if($g->save()):
        if($request->hasFile('file_image')):
          ...
          $img = Image::make($file_file);
          $img->fit(550, 550, function($constraint){
          $constraint->upsize();
          });
          $img->save($upload_path.'/'.$path.'/t_'.$filename);
        endif;
        return back()->with('message', '¡Imagen subida exitosamente!')->with('typealert', 'success');
      endif;
    endif;
  endif;
}
...
public function getProductGalleryDelete($id, $gid) {
  ...
  if($g->product_id != $id) {
    ...
    if($g->delete()):
      // unlik($upload_path.'/'.$path.'/'.$file);
      // unlik($upload_path.'/'.$path.'/t_'.$file);
  ...
  public function postProductSearch(Request $request) {
    ...
    if($validator->fails()):
        return redirect('/admin/products/1')->withErrors($validator)->with('message', 'Oops, ocurrio un error es necesario que cumpla con lo siguiente:')->with('typealert', 'danger')->withInput();
    else:
  ...
  public function getProductDelete($id) {
    ...
    public function getProductRestore($id) {
      ...
      $p->restore();
      if($p->restore()):
        return redirect('/admin/products/'.$p->id.'/edit')->with("message", "Producto restaurado exitosamente")->with("typealert", "success");
      endif;
      }
